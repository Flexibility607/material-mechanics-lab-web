...
### 2. 通过全桥法测 $\varepsilon_F$ 和利用 $\varepsilon_F=\varepsilon_{\max}-\varepsilon_M$ 测 $\varepsilon_F$，哪种方法测量精度高？

- **结论：**全桥法测量 $\varepsilon_F$ 的精度较高。
- **全桥优势：**全桥能直接消除弯曲应变和温度应变的影响，并使有效桥臂的输出叠加。
- **间接测量误差：**若用 $\varepsilon_F=\varepsilon_{\max}-\varepsilon_M$ 间接求取，则两次测量的误差会在相减过程中共同传递到结果中。
